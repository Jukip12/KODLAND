...
def send_welcome(message):
    bot.reply_to(message, "Привет! Я Telegram бот. Напиши команду /hello, /bye, /pass, /emodji или /coin  ")
...
# Получаем файл и сохраняем его
    ...
    if not os.path.isdir('user_img'):
        os.mkdir('user_img')
    os.chdir("user_img")
    if not os.path.isdir(message.from_user.username):
        os.makedirs(message.from_user.username)
    os.chdir("..")
    
    with open(f'user_img/{message.from_user.username}/{file_name}', 'wb') as new_file:
